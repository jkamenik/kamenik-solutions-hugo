async function drawGraph(baseUrl, isHome, pathColors, graphConfig, fetchDataPromise, depthParam) {
  ...
  // Ensure we have valid dimensions - use getBoundingClientRect as fallback
  const containerRect = container.getBoundingClientRect();
  let height = Math.max(container.offsetHeight || containerRect.height || 300, isHome ? 500 : 300);
  let width = container.offsetWidth || containerRect.width || 300;

  // Ensure scale is valid and not zero
  const validScale = scale && !isNaN(scale) && scale > 0 ? scale : 1;
  ...
  // Calculate viewBox values safely
  const viewBoxX = -width / 2 / validScale;
  const viewBoxY = -height / 2 / validScale;
  const viewBoxWidth = width / validScale;
  const viewBoxHeight = height / validScale;

...
Ollama packages model weights, GPU/Metal acceleration, and a simple CLI (`ollama run`, `ollama pull`) behind a local HTTP API. Most integrations use its OpenAI-compatible endpoints so tools can swap `base_url` to `http://localhost:11434` without bespoke SDKs, aligned with our [[personal-automation-ADR-0001-openai-compat-over-litellm|OpenAI-compat provider]] approach for pipelines.
